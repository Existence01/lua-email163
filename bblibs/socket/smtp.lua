...
local tp = require("bblibs.socket.tp")
local ltn12 = require("bblibs.socket.ltn12")
local headers = require("bblibs.socket.headers")
local mime = require("bblibs.socket.mime")
...
local function send_source(mesgt)
    -- make sure we have a content-type iso-8859-1
    local headers = lower_headers(mesgt.headers or {})
...
local function send_string(mesgt)
    ...
    -- 重写该处代码 使其支持HTML格式发送
    headers['content-type'] = headers['content-type'] or
        (_M.HTML and 'text/html; charset="utf8"') or 'text/plain; charset="utf8"'
    send_headers(headers)
